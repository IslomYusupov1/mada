...
@Component({
  selector: 'app-request-param-title-translate',
  templateUrl: './request-param-title-translate.component.html',
  styleUrls: ['./request-param-title-translate.component.scss']
})
export class RequestParamTitleTranslateComponent implements OnInit {
  ...
  constructor(
    ...
    private dialogRef: MatDialogRef<RequestParamTitleTranslateComponent>,
  ...
  saveTranslate() {
    if (this.titleTranslationForm.valid) {
      this.paymentService.serviceParamsTranslates({
        titleTranslate: this.titleTranslationForm.value,
        requestParamId: this.data.uuid
      }).then((res: any) => {
        if (res) {
